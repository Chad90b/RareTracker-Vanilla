...
-- update 2024.12.05, corrected issue #1 with the scan button not being functional

RareTracker = {
	version = "1.5.4",
	supportedbuild = "11000",
	lastupdate = "Dec 05, 2024",
	author = "Silent",
...
--[[CHAD 2024.05.12 replacing this entire section to clean it up a little
--OLD below
...
function RareTracker:Scan()
	...
	for name, _ in pairs(mobs) do
		...
		-- restore previous target or clear target
		if currentTarget and (not newTarget or newTarget ~= currentTarget) then
			TargetLastTarget()
		elseif newTarget and newTarget ~= currentTarget then
			ClearTarget()
		end
	end

	-- target the rare if found and conditions are met
	if TNE_RareTrackerCore_AutoTarget and rareTarget and not (currentTarget or dead) then
		TargetByName(rareTarget)
	end

	-- display an error message if no rares were found
	if not rareTarget then
		UIErrorsFrame:AddMessage(RT_SCAN_FAILED, 1, 0, 0, 1, UIERRORS_HOLD_TIME)
	end

	-- restore the error frame
	RareTracker:ReleaseUIErrorsFrame()
end -- RareTracker:Scan() end
